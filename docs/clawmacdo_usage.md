...
Before running this command, complete these one-time steps in the [Tailscale admin console](https://login.tailscale.com/admin):

1. **Enable Funnel on your tailnet.** Go to [DNS → Funnel](https://login.tailscale.com/admin/dns) and enable Funnel. Without this, step 3 will hang waiting for approval.
2. **Generate an auth key** (not an API key). Go to [Settings → Keys](https://login.tailscale.com/admin/settings/keys) → **"Generate auth key..."**. Check **"Reusable"** so it can be used across multiple deployments. The key starts with `tskey-auth-`. Do **not** use an API key (`tskey-api-`) — API keys are for the Tailscale REST API and cannot authenticate devices.

> **Key types:** `tskey-auth-...` = auth key (registers devices to your tailnet). `tskey-api-...` = API key (manages your tailnet via REST API). This command requires an **auth key**.
...
- **Funnel toggle** — Each deployment row has an On/Off button to enable/disable Tailscale Funnel, showing the public URL when active. Requires a Tailscale **auth key** (`tskey-auth-...`) saved in the user's Settings page. Funnel must also be [enabled on the user's tailnet](https://login.tailscale.com/admin/dns) before first use
...
| Variable | Used by | Description |
|----------|---------|-------------|
| `TAILSCALE_AUTH_KEY` | tailscale-funnel | Tailscale **auth** key (`tskey-auth-...`). Must be an auth key, not an API key (`tskey-api-...`). In the web UI, each user provides their own key via Settings for network isolation. |
